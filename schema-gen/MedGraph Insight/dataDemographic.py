...
def fill_dscrp(row):
    if not row['Description']:
        ...
        if value == 1:
            ...
            additional_values = list(np.random.choice(
                [2, 3, 4, 5], np.random.randint(1, 4), replace=False))
...
illness_dscrp = {1: 'No known illness', 2: 'Hypertension',
                 3: 'Diabetes', 4: 'Arthritis', 5: 'Cardio bypass'}
df_demographics['Description'] = df_demographics['Description'].apply(
    lambda x: [illness_dscrp[i] for i in x])

# Join descriptions for each row
df_demographics['Description'] = df_demographics['Description'].apply(
    lambda x: x[0] if x[0] == 'No known illness' else 'History of ' + ', '.join(map(str, x)))

# Create a new column '_id' with specified values for the first 6 rows
df_demographics['_id'] = ''
df_demographics.loc[:5, '_id'] = ['63701d24f03239c72c00018e', '63701d24f03239c72c00018f',
                                  '63701d24f03239c72c000190', '63701d24f03239c72c000191', 
                                  '63701d24f03239867500012a', '63701d24f03239867500012b']

# Generate random IDs for the rest of the rows
for i in range(6, 300):
    random_id = ''.join(np.random.choice(
        list('0123456789abcdef'), len(df_demographics['_id'].iloc[0])))
...
df_demographics.to_excel(os.path.join(
    new_path, 'dataDemographics.xlsx'), index=False)
